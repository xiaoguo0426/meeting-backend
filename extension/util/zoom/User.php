...
class User extends Zoom
{
    ...
    public function lists(string $role_id = '0', string $status = 'active', int $page_size = 30, int $page_number = 1, string $next_page_token = '')
    {
        try {
            $query = [
                'status' => $status,
    ...
    public function get(string $userIdOrEmail)
    {
        try {
            $query = [
            ];
            $response = $this->request('/v2/users/' . $userIdOrEmail, $query);
            return json_decode($response->getBody()->getContents(), true);
        } catch (GuzzleException $e) {
            $response = $e->getResponse();
            $body = json_decode($response->getBody(), true);
