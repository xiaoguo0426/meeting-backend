...
require.config({
    ...
    paths: {
        ...
        'editormd': ['plugs/editor/editormd.amd.min'],
    ...
    shim: {
        ...
        'editormd.amd': {
            deps: [
                'jquery',
                baseRoot + 'plugs/editor/editormd.js',
                baseRoot + 'plugs/editor/languages/en.js',
                baseRoot + 'plugs/editor/plugins/link-dialog/link-dialog.js',
                baseRoot + 'plugs/editor/plugins/reference-link-dialog/reference-link-dialog.js',
                baseRoot + 'plugs/editor/plugins/image-dialog/image-dialog.js',
                baseRoot + 'plugs/editor/plugins/code-block-dialog/code-block-dialog.js',
                baseRoot + 'plugs/editor/plugins/table-dialog/table-dialog.js',
                baseRoot + 'plugs/editor/plugins/emoji-dialog/emoji-dialog.js',
                baseRoot + 'plugs/editor/plugins/goto-line-dialog/goto-line-dialog.js',
                baseRoot + 'plugs/editor/plugins/help-dialog/help-dialog.js',
                baseRoot + 'plugs/editor/plugins/html-entities-dialog/html-entities-dialog.js',
                baseRoot + 'plugs/editor/plugins/preformatted-text-dialog/preformatted-text-dialog.js',
                'css!' + baseRoot + 'plugs/editor/css/editormd.css'
            ],
        },
    }
});
...
$(function () {
    ...
    $.msg = new function () {
        ...
        this.loading = function (msg, call) {
            var idx = msg ? layer.msg(msg, {
                icon: 16,
                scrollbar: false,
                shade: this.shade,
                time: 0,
                end: call
            }) : layer.load(2, {time: 0, scrollbar: false, shade: this.shade, end: call});
            return that.idx.push(idx), idx;
        };
        /*! 页面加载层 */
        ...
        this.confirm = function (msg, ok, no) {
            return layer.confirm(msg, {title: 'confirm', btn: ['YES', 'NO']}, function (idx) {
    ...
    $.menu = new function () {
        ...
        this.queryNode = function (url, node) {
            if (!/^m-/.test(node = node || location.href.replace(/.*spm=([\d\-m]+).*/ig, '$1'))) {
                ...
                console.log('[data-menu-node][data-open*="' + url.replace(/\.html$/ig, '') + '"]')
                console.log($menu);
        ...
        this.parseUri = function (uri, elem, vars, temp, attrs) {
            ...
            console.log(uri);
            uri = this.getUri(uri);console.log(vars);
            console.log(uri);
        ...
        this.listen = function () {
            ...
            onEvent('click', '[data-target-menu-type]', function () {
                layui.data('admin-menu-type', {
                    key: 'type-mini',
                    value: $menu.toggleClass(miniClass).hasClass(miniClass)
                });
            });
            /*! 监听窗口尺寸处理 */
            (function (callable) {
            ...
            this.syncOpenStatus = function (mode) {
                $('[data-submenu-layout]').map(function () {
                    ...
                    if (mode === 1) layui.data('admin-menu-stat', {
                        key: node,
    ...
    $.vali = function (form, callable) {
        ...
        function Validate() {
            ...
            this.hasProp = function (ele, prop) {
                ...
                var $next = $(ele).nextAll('.input-right-icon'),
                    right = ($next ? $next.width() + parseFloat($next.css('right') || '0') : 0) + 10;
                var style = {
                    top: $(ele).position().top + 'px',
                    right: right + 'px',
                    lineHeight: ele.nodeName === 'TEXTAREA' ? '32px' : $(ele).css('height')
                };
    ...
    $.fn.uploadOneImage = function () {
        return this.each(function () {
            ...
            var $in = $(this),
                $bt = $('<a data-file class="uploadimage transition"><span class="layui-icon">&#x1006;</span></a>').data('input', this);
    ...
    $.fn.uploadMultipleImage = function () {
        return this.each(function () {
            if ($(this).data('inited')) return true; else $(this).data('inited', true);
            var $in = $(this), $bt = $('<a data-file="mul" class="uploadimage"></a>'),
        ...
        return this.each(function (idx, elem) {
            ...
            function bindData(opts) {
                ...
                if (sort.field && sort.type) {
                    ...
                    opts.initSort = {
                        type: sort.type.split(',')[0].split(' ')[0],
                        field: sort.field.split(',')[0].split(' ')[0]
                    };
                }
    ...
    $.previewPhonePage = function (href, title) {
        ...
        layer.style(layer.open({
            ...
            content: layui.laytpl(template).render({title: title || '公众号', url: href})
        }), {boxShadow: 'none'});
    };
            ...
            success: function ($elem) {
                new function () {
                    ...
                    this.$percent = this.$box.find('.layui-progress div'), this.SetCache = function (code, index, value) {
                        ...
                        return value !== undefined ? layui.data(ctype, {
                            key: ckey,
                            value: value
                        }) : layui.data(ctype)[ckey] || 0;
    ...
    onEvent('click', '[data-copy]', function () {
        ...
        if (window.clipboardData) {
            ...
            $.msg.tips(document.execCommand('Copy') ? 'Coped！' : 'Please use the right mouse button to copy ！') && $input.remove();
        }
    });
